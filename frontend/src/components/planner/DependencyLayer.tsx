...
export default function DependencyLayer({ scrollRef }: Props) {
  const { project, resolution, gridStart, depDrawing, drag } = useStore()
  ...
  // Build effective task list — replace dragged task with its ghost position/member
  const effectiveTasks: Task[] = project.tasks.map(t => {
    if (drag && t.id === drag.taskId) {
      return {
        ...t,
        startTime: drag.ghostStartTime,
        memberId: drag.ghostMemberId,
        laneRow: drag.ghostRow,
      }
    }
    return t
  })

  // Build cumulative Y offsets per member using effective tasks
  ...
  for (const task of effectiveTasks) {
    for (const dep of task.children) {
      ...
      const x1 = MEMBER_COLUMN_WIDTH + msToPixels(fromTask.startTime + fromTask.duration * 60 * 1000 - gridStart, resolution)
  ...
  // Live arrow while drawing a new dependency
  const getLiveArrow = () => {
